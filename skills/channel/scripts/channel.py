...
def create_draft(access_token: str, title: str, content: str, author: str = "",
                cover_image: str = None, thumb_media_id: str = None,
    ...
    # 上传封面图（如果有）
    if cover_image and os.path.exists(cover_image) and not thumb_media_id:
        # 封面图需要通过素材上传接口
    ...
    articles = [{
        ...
        "show_cover_pic": 1 if (cover_image or thumb_media_id) else 0
    }]
...
示例:
  ...
  %(prog)s delete media_id_here
    ...
    create_parser.add_argument('--thumb-id', help='直接指定封面图 Media ID')
