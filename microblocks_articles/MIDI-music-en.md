---
title: "Live music with MIDI devices"
url: "https://learn.microblocks.fun/en/activities/MIDI-music-en"
slug: "MIDI-music-en"
---
...
should be of 10Ω.
 ...
 Some boards have a hardware serial pin. Look up which one it is in the
pinout diagram for your particular board. Otherwise, you can use any digital pin
as a software serial pin, but hardware serial may perform better.
...
board, you're all set. Otherwise, keep on reading to learn what the block does.

#### Software serial

To set up your board to send MIDI commands through a software serial port, just
input the correct pin number into the block and click on it.
...
The block expresses what we call a whole note. The
rest of the duration blocks express fractions of a whole note. For instance,
 represents a half-note (or half of a whole note), while
...
start by using the . You can then check what
the current beat and bar are by using the and
 blocks, and use the corresponding wait blocks
to sync your music to the beat or bar that you need.

#### Synchronizing beats

As we've seen before, synchronizing scripts that make music can get tricky. The
purpose of the block is to make our life
...
Notice how they never go out of sync? The trick lies in the fact that the
 block makes use of a metronome of sorts,
...
The block will always make sure to keep
them in sync.
...
activity we're just going to use the more explicit .

The MIDI library block
can take chords as inputs in addition to just single notes, so if you drop the
 into the note input, it will just play it. In this
...
In this example, the block returns 36, 40, 43, 48.
...
just equate a bar to a duration. That is, 2 bars will
...
Notice how we're waiting until this time. This makes
...
When that happens, the next may have to
...
the time it was supposed to, and thus had
...
whole notes, we can ask to wait until,
well, the next time period.
